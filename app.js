...
$("magFile").addEventListener("change",e=>{
 ...
 readMatrix(f).then(m=>{
   ...
   build();
 }).catch(err=>showError("Errore file Magazzino: "+err.message));
});
$("sizeFile").addEventListener("change",e=>{
 ...
 readMatrix(f).then(m=>{
   ...
   build();
 }).catch(err=>showError("Errore file SIZE: "+err.message));
});
function readMatrix(file){
 return new Promise((resolve,reject)=>{
  const r=new FileReader();
  r.onload=e=>{try{
    if(typeof XLSX==="undefined") throw new Error("Libreria Excel non caricata. Controlla la connessione Internet di GitHub Pages.");
...
function n(v){
 ...
 const x=parseFloat(s.replace(/[^\d.-]/g,"")); return Number.isFinite(x)?x:0;
}
function norm(v){return text(v).normalize("NFD").replace(/[\u0300-\u036f]/g,"").replace(/\s+/g," ").toUpperCase()}
function findCol(row,name){const target=norm(name);return row.findIndex(v=>norm(v)===target)}
function parseMag(m){
 // Nel report reale: riga 11 contiene le intestazioni; ogni prodotto occupa due righe:
 // riga codice/U.M./valori e riga successiva con il nome prodotto.
 let header=-1;
 for(let i=0;i<m.length;i++) if(m[i].some(v=>norm(v)==="OPENING BALANCE")){header=i;break}
...
function parseSize(m){
 // Nel file SIZE reale: prima riga utile di intestazione è la riga con PRODOTTO.
 let h=-1;
 for(let i=0;i<m.length;i++) if(m[i].some(v=>norm(v)==="PRODOTTO")){h=i;break}
 if(h<0) throw new Error("Colonna PRODOTTO non trovata nel file SIZE.");
 const head=m[h], p=findCol(head,"PRODOTTO"), box=findCol(head,"BOX"), sl=findCol(head,"SLEEVE"), sg=findCol(head,"C.B. SINGOLO P.");
 const out=[];
 for(let i=h+1;i<m.length;i++){
   const r=m[i], name=text(r[p]); if(!name) continue;
...
function render(){
 ...
 data.forEach(r=>{
   ...
   inp.addEventListener("input",()=>{const v=n(inp.value),x=v-r.atteso;d.textContent=fmt(x);d.className="num diff "+(x===0?"ok":"bad")});
   $("body").appendChild(tr);
 });
}
